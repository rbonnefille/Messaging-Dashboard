<template>
  <div class="mb-4 container">
          ...
          <div class="card">
            ...
            <ul class="list-group list-group-flush">
              <VDataItem
                ...
                :value="suncoUser?.id" />
              <VDataItem
                v-if="suncoUser?.externalId"
                :loading="isLoading"
                label="External id"
                :value="suncoUser?.externalId" />
              <VDataItem
                v-if="suncoUser?.zendeskId"
                :loading="isLoading"
                label="zendeskId"
                :value="suncoUser?.zendeskId" />
              <VDataItem
                v-if="suncoUser?.signedUpAt"
                :loading="isLoading"
                label="signedUpAt"
                :value="suncoUser?.signedUpAt" />
              <VDataItem
                v-if="suncoUser?.toBeRetained"
                :loading="isLoading"
                label="toBeRetained"
              ...
              <VDataItem
                :loading="isLoading"
                label="authenticated"
                :value="suncoUser?.authenticated" />
              <ul
                v-if="Object.keys(suncoUser?.profile || {}).length > 0"
              ...
              <ul
                v-if="suncoUser?.identities.length > 0"
                class="list-group-item">
                identities:
                <template v-if="isLoading">
                  <VSkeleton v-for="n in 3" :key="n" :loading="isLoading" />
                ...
                <template v-else>
                  <template
                    v-for="(identities, index) in suncoUser?.identities"
                    :key="index">
                    <template
    ...
    <template v-if="suncoUserDevices?.length > 0">
      <h2
        class="mt-3 collapsable"
        data-bs-toggle="collapse"
        href="#collapseDevices"
        role="button"
        aria-expanded="false"
        aria-controls="collapseDevices"
        @click="devicesCollapsed = !devicesCollapsed">
        <span
    ...
    <template v-if="suncoUserConversations?.length > 0">
      <h2
        class="mt-3 collapsable"
        data-bs-toggle="collapse"
        href="#collapseConversations"
        role="button"
        aria-expanded="false"
        aria-controls="collapseConversations"
      ...
      <VButton
        v-if="suncoUserConversations?.length >= 2"
        @click="deleteConversations"
        type="button"
        class="btn btn-danger"
        text="Delete all conversations not associated with a ticket" />
      <div class="row row-cols-3 collapse" id="collapseConversations">
        <div
          v-for="(conversation, key) in conversationsByTime"
          :key="key"
          class="col">
          <div v-if="conversation" class="card">
            ...
            <ul class="list-group list-group-flush">
              ...
              <VDataItem
                :loading="isLoading"
                label="isDefault"
                :value="conversation.isDefault"
                :class-name="[
                  { 'fs-7 fw-bold text-success': conversation.isDefault },
                ]" />
              <VDataItem
                v-if="conversation.brandId"
                :loading="isLoading"
                label="brandId"
              ...
              <template v-if="conversation.metadata">
                <ul class="list-group-item">
                  metadata:
                  <template
                    v-for="(metadata, metadataKey) in conversation.metadata"
                    :key="metadataKey">
                    <VDataItem
                      :loading="isLoading"
                      :label="metadataKey"
              ...
              <ul
                v-if="conversation.activeSwitchboardIntegration"
                class="list-group-item">
                activeSwitchboardIntegration:
                <template
                  v-for="(
                    sbIntegration, sbIntegrationKey
                  ) in conversation.activeSwitchboardIntegration"
                  :key="sbIntegrationKey">
                  <VDataItem
                    :loading="isLoading"
                    :label="sbIntegrationKey"
                    :value="sbIntegration" />
                </template>
              </ul>
              <VDataItem
                ...
                :value="conversation.businessLastRead" />
              <VDataItem
                :loading="isLoading"
                label="lastUpdatedAt"
                :value="conversation.lastUpdatedAt" />
            </ul>
          </div>
        </div>
      </div>
